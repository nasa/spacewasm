...
int clock_ms(struct spacewasm_caller_t *caller,
              void *userdata,
    ...
    out_result->tag = SPACEWASM_I64;
    out_result->u.i64_ = ((uint64_t)ts.tv_sec * 1000) + ((uint64_t)ts.tv_nsec / 1000000);
...
int main(void) {
    ...
    spacewasm_status_t st = spacewasm_host_new(1, &host);

    uint32_t env_module_index;
    spacewasm_add_host_module(&host, "env", 1, 0, &env_module_index);
    spacewasm_add_host_function(&host, env_module_index, "clock_ms", "", "I", &clock_ms, NULL);
    ...
    uint32_t idx = 0;
    st = spacewasm_find_export_func(store, mod_idx, "run", &idx);
    if (st != SPACEWASM_OK) {
