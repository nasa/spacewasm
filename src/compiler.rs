...
impl<const MAX_CONTROL_FRAMES: usize, const MAX_STACK_DEPTH: usize> WasmVisitor
    for Compiler<'_, MAX_CONTROL_FRAMES, MAX_STACK_DEPTH>
{
    ...
    fn global_get(&self, x: GlobalIdx, state: &mut Self::State) -> Result<(), Self::Error> {
        ...
        match g.reference {
            Ref::Module(idx) => {
                state.instr_imm_8_or_16(GLOBAL_GET, u32::from(idx))?;
                Ok(())
            }
            Ref::Extern { module, index } => {
    ...
    fn global_set(&self, x: GlobalIdx, state: &mut Self::State) -> Result<(), Self::Error> {
        ...
        if g.mutable {
            ...
            match g.reference {
                Ref::Module(idx) => {
                    state.instr_imm_8_or_16(GLOBAL_SET, u32::from(idx))?;
                    Ok(())
                }
                Ref::Extern { module, index } => {
