...
function colorize(new_value, old, lower_better) {
    ...
    if (old != new_value) {
        ...
        if (pct_change > 0) {
            ...
            change_text = "(↑" + str(pct_change) + "% from " + str(old) + ")";
        } else {
            color = lower_better ? "green" : "red";
            change_text = "(↓" + str(-pct_change) + "% from " + str(old) + ")";
        }
    }
    return "$${\\color{" + color + "}{" + str(new_value) + "}}$$<br>" + change_text;
}
...
if (benchmark) {
    ...
    let triples = benchmark.currentScore.triples;
    let elf_sections = benchmark.currentScore.elf_sections;

    let comment = "# ELF Section Sizes";
    comment += "||" + elf_sections.map(i => "`"+i+"`").join("|") + "|";
    comment += "|--:|" + ":-:|" * triples.length;
    ...
    comment += "<details><summary><i>view detailed section sizes...</i></summary>\n";
    for (let triple of triples) {
        comment += "### `"+triple+"`";
        comment += "```\n"+data[triple]["details"]+"\n```\n";
    }
    ...
    for (let triple of triples) {
        ...
        comment += "|**`"+triple+"`**|"+qemu_info[triple]+"|"+colorize(score, old_score, false)+"|"+t.toFixed(2)+" s|";
    }
    comment += "\n";
